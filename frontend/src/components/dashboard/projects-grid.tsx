    ...
    return (
      <Card>
        <CardContent className="flex flex-col items-center justify-center py-12">
          ...
          <h3 className="text-lg font-semibold text-gray-900 dark:text-white mb-2">{t('dashboard.noProjects')}</h3>
          <p className="text-gray-500 dark:text-gray-400 mb-4">{t('dashboard.createFirst')}</p>
          <Link href="/projects/new">
            <Button>{t('dashboard.addProject')}</Button>
          </Link>
        </CardContent>
      </Card>
    );
        ...
        <Card key={project.id} className="hover:shadow-lg transition-shadow">
          ...
          <CardContent>
            ...
            <div className="grid grid-cols-3 gap-4 mb-4">
              <div>
                <p className="text-xs text-gray-500 dark:text-gray-400">{t('projects.endpoints')}</p>
                <p className="text-lg font-semibold">{project.endpointCount}</p>
              </div>
              <div>
                <p className="text-xs text-gray-500 dark:text-gray-400">{t('common.success')}</p>
                <p className="text-lg font-semibold text-green-500">{project.successCount}</p>
              </div>
              <div>
                <p className="text-xs text-gray-500 dark:text-gray-400">{t('common.failed')}</p>
